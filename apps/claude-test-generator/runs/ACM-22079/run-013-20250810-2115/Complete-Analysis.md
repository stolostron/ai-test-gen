...
#### Setup
**Prerequisites**:
- Hub cluster login: `oc login <hub-cluster-api-url> -u <username> -p <password> --insecure-skip-tls-verify`
...
|-------|-----------------|
| **Step 1: Environment Validation**<br/><br/>Instructions:<br/>1. Go to your terminal<br/>2. Run the following command: `oc login <hub-cluster-api-url> -u <username> -p <password> --insecure-skip-tls-verify && oc whoami && oc get managedclusters` | **Success Criteria**: Authenticated connection to ACM hub with managed clusters visible<br/><br/>**Expected Output**:<br/>```<br/>Login successful.<br/><username><br/>NAME                          HUB ACCEPTED   JOINED   AVAILABLE   AGE<br/>local-cluster                 true           True     True        2d<br/>managed-cluster-1             true           True     Unknown     2d<br/>managed-cluster-2             true           Unknown             2d<br/>```<br/><br/>*Confirms hub cluster access and managed cluster availability for testing* |
...
## Deployment Assessment & Testing Strategy

### Current Environment Status
**Environment**: ACM test cluster operational with managed clusters available  
**Feature Status**: ACM-22079 digest-based upgrade functionality not yet deployed  
**Test Readiness**: Complete E2E test plan ready for execution

### Feature Deployment Readiness Assessment

**Implementation Status:**
- **ACM-22079 Core Feature**: Implementation in progress
- **validateUpgradeVersion Enhancement**: NEW function to return digest strings
- **Annotation Security Logic**: NEW security gating for non-recommended versions
- **Fallback Algorithm**: NEW fallback chain implementation

**Testing Approach:**
1. **Current State**: Test plan prepared and validated
2. **Deployment Phase**: Execute tests when feature is deployed to test environment
3. **Validation Phase**: Complete E2E validation of all digest functionality
4. **Production Ready**: Tests confirm feature meets Amadeus requirements

### Expected Test Execution Outcomes

**When Feature is Deployed:**
- **Test Case 1**: Validates complete digest-based upgrade workflow
- **Test Case 2**: Confirms fallback mechanism reliability  
- **Test Case 3**: Verifies security controls and authorization

**Current Limitations:**
- Feature implementation not yet available in test environment
- Cannot execute digest-specific functionality until deployment
- Standard ClusterCurator operations remain functional

---

## Business Value & Customer Impact

### Amadeus Customer Requirements
**Critical Business Need**: Disconnected environment support for OpenShift upgrades
**Technical Challenge**: Image tags don't resolve in air-gapped environments
**Solution Provided**: Digest-based upgrade support with robust fallback mechanisms

### Value Proposition
1. **Reliability**: Digest-based upgrades provide exact version matching
2. **Robustness**: Multi-source fallback ensures operation in various scenarios
3. **Security**: Annotation-based controls prevent unauthorized upgrades
4. **Production Ready**: Complete E2E validation ensures customer success
