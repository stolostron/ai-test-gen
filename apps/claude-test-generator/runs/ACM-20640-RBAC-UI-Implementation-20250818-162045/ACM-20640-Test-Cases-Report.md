...
| 3 | **Create New User Identity** - Create a new user for RBAC testing | Click "Create User" → Fill user details form (name, email, groups) → Click "Create" | Create user YAML file: `touch test-user.yaml` and add:
```yaml
...
metadata:
  name: rbac-test-user
  labels:
    rbac.acm.io/test-user: "true"
identities:
- htpasswd:rbac-test-user
```
`oc apply -f test-user.yaml` | New user appears in users list with "Active" status and proper metadata |
...
| 5 | **Edit User Profile** - Modify user information and group assignments | Click "Edit User" → Update user groups and labels → Save changes | Edit existing user YAML:
```yaml
apiVersion: user.openshift.io/v1
kind: User
metadata:
  ...
  labels:
    rbac.acm.io/test-user: "true"
    rbac.acm.io/department: "qa"
identities:
- htpasswd:rbac-test-user
groups:
- qa-team
```
`oc apply -f test-user.yaml` | User profile updates successfully with new group assignments reflected immediately |
...
| 3 | **Launch Assignment Wizard** - Start new role assignment creation process | Click "Create RoleAssignment" → Role Assignment Wizard opens | Create RoleAssignment YAML: `touch roleassignment-vm-admin.yaml` and add:
```yaml
apiVersion: rbac.open-cluster-management.io/v1alpha1
kind: RoleAssignment
metadata:
  name: vm-admin-assignment
  namespace: target-cluster
spec:
  role: kubevirt.io:admin
  subjects:
  - kind: User
    name: rbac-test-user
    clusters:
    - name: target-cluster
      clusterWide: false
      namespaces: ["default", "vm-workloads"]
```
`oc apply -f roleassignment-vm-admin.yaml` | Role Assignment Wizard displays with clear step-by-step process and validation |
...
| 9 | **Edit Existing Assignment** - Modify role assignment scope and permissions | Click assignment actions → "Edit" → Modify namespace scope → Save changes | Edit RoleAssignment YAML to add namespace:
```yaml
      namespaces: ["default", "vm-workloads", "production"]
```
`oc apply -f roleassignment-vm-admin.yaml` | Assignment updates successfully with expanded namespace access |
...
| 4 | **Create VM-Scoped Role Assignment** - Assign VM management roles to users | Click "Create VM Role Assignment" → Select VM admin role → Target specific VMs | Create VM RoleAssignment: `touch vm-rbac-assignment.yaml` and add:
```yaml
apiVersion: rbac.open-cluster-management.io/v1alpha1
kind: RoleAssignment
metadata:
  name: vm-lifecycle-admin
...
spec:
  ...
  - kind: User
    ...
    - name: production-cluster
      clusterWide: false
      namespaces: ["vm-workloads"]
      resources:
      - virtualmachines
      - virtualmachineinstances
```
`oc apply -f vm-rbac-assignment.yaml` | VM role assignment created with granular VM operation permissions |
...
| 3 | **Create Cross-Cluster Role Assignment** - Assign roles that span multiple clusters | User Management → Create RoleAssignment → Select multiple target clusters | Create cross-cluster RoleAssignment: `touch cross-cluster-rbac.yaml` and add:
```yaml
apiVersion: rbac.open-cluster-management.io/v1alpha1
kind: RoleAssignment
metadata:
...
spec:
  ...
  - kind: User
    ...
    - name: staging-cluster
      clusterWide: false
      namespaces: ["default", "testing"]
```
`oc apply -f cross-cluster-rbac.yaml` | Cross-cluster role assignment created with proper cluster targeting |
...
| 2 | **Test Invalid Role Assignment** - Attempt to create role assignment with invalid configuration | Create RoleAssignment → Enter invalid role name → Attempt submission | Create invalid RoleAssignment: `touch invalid-rbac.yaml` and add:
```yaml
apiVersion: rbac.open-cluster-management.io/v1alpha1
kind: RoleAssignment
metadata:
...
spec:
  ...
  - kind: User
    name: test-user
```
`oc apply -f invalid-rbac.yaml` | Clear error message displayed with specific validation failure details |
...
spec:
  ...
  - kind: User
    ...
    - name: production
      clusterWide: true
```
`oc apply -f conflicting-rbac.yaml` | System properly handles conflicts with appropriate precedence and user notification |
...
| 5 | **Validate YAML Editor Error Handling** - Test error handling in YAML configuration mode | Switch to YAML editor → Enter malformed YAML → Attempt save | Create malformed YAML:
```yaml
apiVersion: rbac.open-cluster-management.io/v1alpha1
kind: RoleAssignment
metadata:
...
```
`oc apply -f malformed-rbac.yaml` | YAML validation errors clearly displayed with line numbers and correction suggestions |
...
| 7 | **Verify Form Validation** - Test comprehensive form field validation and user guidance | Fill role assignment form with invalid data → Test each field validation | Attempt CLI with missing required fields:
```bash
oc create roleassignment --dry-run=client
```
Review required field validation | Form validation provides immediate feedback with clear correction guidance |
| 8 | **Test Bulk Operation Error Handling** - Verify error handling during multiple role assignments | Perform bulk role assignment operations with mixed valid/invalid entries | Apply multiple assignments including failures:
```bash
oc apply -f valid-rbac.yaml,invalid-rbac.yaml
```
Review batch operation results | Bulk operations report individual successes/failures with detailed error breakdown |
